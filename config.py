...
# ── 진입 품질 필터 (2026-04-13 손실 분석 반영) ──
# [수정1] ATR이 진입가 대비 이 비율 미만이면 진입 차단
# 예) 진입가 1,407원 × 0.008 = 11.3원 → ATR 4.5원(0.32%)은 차단
# 예) 진입가 8,470원 × 0.008 = 67.8원 → ATR 314.3원(3.7%)은 통과
MIN_ATR_RATIO = 0.008           # 진입가 대비 0.8%

# [수정2] 체결가 기준 손절여유 최솟값
# 슬리피지 후 (체결가 - 손절선) / 체결가 가 이 비율 미만이면 즉시 청산
# 예) 뉴엔AI: 체결 15,640 손절 15,340 → 여유 1.92% → 통과
# 예) 슬리피지 극심: 체결 ≈ 손절선 → 차단
MIN_FILL_MARGIN_RATIO = 0.005   # 0.5%
...
MINI_TRAIL_GAP      = 0.010   # 고점 대비 -1.0% 하락 시 청산 (09:10 이후)
